...
server <- function(input, output) {
    ...
    output$v2gTable <- renderReactable({
        ...
        if (is.null(data)) {
            ...
            ), searchable = TRUE, resizable = TRUE, filterable = TRUE,
            paginationType = "jump", defaultPageSize = 10, pageSizeOptions = c(10, 25, 50, 100, 200))
        }
    })
    ...
    output$filterUI <- renderUI({
        ...
        if (!is.null(data)) {
            tagList(
                filter_select("filterRsidInput", "Filter by rsid (input)", data, ~variant_input),
